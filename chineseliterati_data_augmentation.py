...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a multi-label emotion classifier on Chinese Literati images.")
    parser.add_argument("--csv", type=Path, required=True, help="Path to labels CSV file.")
    parser.add_argument("--images", type=Path, required=True, help="Path to images directory.")
    parser.add_argument("--output", type=Path, default=Path("outputs_literati"), help="Directory to store logs and checkpoints.")
...
class ChineseLiteratiDataset(Dataset):
...
def build_dataloaders(csv_path, images_path, batch_size, val_split, seed, emotions=None):
    ...
    base_ds = ChineseLiteratiDataset(csv_path, images_path, transform=None)
    if emotions is None:
    ...
    train_idx, val_idx = random_split(range(len(base_ds)), [num_train, num_val], generator=g)

    train_ds = Subset(ChineseLiteratiDataset(csv_path, images_path, transform=train_tfms, emotions=emotions),
                      train_idx.indices if hasattr(train_idx, "indices") else train_idx)
    val_ds = Subset(ChineseLiteratiDataset(csv_path, images_path, transform=val_tfms, emotions=emotions),
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch}/{args.epochs} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | "
              f"Micro F1: {micro_f1:.4f} | Macro F1: {macro_f1:.4f} | mAP: {mAP:.4f}")
